...
public class MovieDAO {
    ...
    private static final String selectMovieByIdQuery = "select * from movies where id = ?";
    ...
    public static Movie getMovie(int id) throws MovieException {
        Movie movie=new Movie();
        try(Connection connection = JdbcConnection.getConnection()) {
            PreparedStatement ps = connection.prepareStatement(selectMovieByIdQuery);
            ps.setInt(1, id);
            ResultSet rs=ps.executeQuery();
            if(rs.next()){
               movie=getMovie(rs);
